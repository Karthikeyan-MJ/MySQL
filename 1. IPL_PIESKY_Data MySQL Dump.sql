...
select  bd2.bidder_id, bd1.bidder_name, bd2.bid_status,
sum(case 
bd2.bid_status when 'won' then 1 else 0 
end) as No_of_wins,
(sum(case 
bd2.bid_status when 'won' then 1 else 0 
end)/count(*))*100 as winning_percentage
from ipl_bidder_details bd1
inner join ipl_bidding_details bd2
on bd1.bidder_id =  bd2.bidder_id
group by bd1.bidder_name, bd2.bidder_id
order by (sum(case bd2.bid_status when 'won' then 1 else 0 end)/count(*))*100 desc;

#2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
select * from ipl_match_schedule;
select * from ipl_stadium;
select stadium_name, city, stadium_id, count(ipl_match_schedule.match_date)
from ipl.ipl_stadium
Natural join ipl.ipl_match_schedule
group by stadium_id;
...
select bdg.bidder_id, bdr.bidder_name, bdg.bid_team, count(bp.no_of_bids) as Total_bids
from ipl_bidding_details bdg
inner join ipl_bidder_details bdr
on bdr.bidder_id = bdg.bidder_id
inner join ipl_bidder_points bp
on bp.bidder_id = bdg.bidder_id
inner join ipl_match_schedule ms
on bdg.schedule_id = ms.schedule_id
inner join ipl_match m
on m.match_id = ms.match_id
inner join ipl_team t
on t.team_ID = m.team_ID1
group by bdr.bidder_name;
...
select ipl.ipl_match.match_id, ipl.ipl_match.WIN_DETAILS,
case
when ipl.ipl_match.match_winner = 1 then ipl.ipl_match.team_id1 
else ipl.ipl_match.team_id2
end as team_id
from ipl.ipl_match
inner join ipl.ipl_team
on ipl.ipl_match.team_id2 = ipl.ipl_team.team_id
group by ipl.ipl_match.win_details;


#6.	Display total matches played, total matches won and total matches lost by team along with its team name.

select t.team_name, sum(ts.matches_played), sum(ts.matches_won), sum(ts.matches_lost)
from ipl_team t
inner join ipl_team_standings ts
...
select t.team_name,tp.player_role,count(*) as ALL_ROUNDERS
